...
% Concatenar imagens e rótulos de ambos os conjuntos de dados
allImages = cat(4, startImages, trainImages);
allLabels = vertcat(startLabels, trainLabels);

% Converter rótulos para dados categóricos
allLabels = categorical(allLabels, 1:numel(classFolders), classFolders);
trainLabels = categorical(trainLabels, 1:numel(classFolders), classFolders);
...
net = trainNetwork(allImages, allLabels, layers, options);
...
save('models/trained-model-4.mat', 'net');