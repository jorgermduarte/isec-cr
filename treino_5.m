...
startLabels = categorical(startLabels, 1:numel(classFolders), classFolders);
...
options = trainingOptions('sgdm', ...
    ...
    'ValidationData', {startImages, startLabels}, ...
    'ValidationFrequency', 30);

net = trainNetwork(trainImages, trainLabels, layers, options);

% Testar a rede com imagens da pasta "start"
YPred = classify(net, startImages);
accuracy = sum(YPred == startLabels) / numel(startLabels);
fprintf('Accuracy: %.2f\n', accuracy);

% Salvar a rede treinada
save('models/trained-model-5.mat', 'net');
